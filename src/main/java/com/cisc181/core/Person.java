...
public abstract class Person implements java.io.Serializable {
			...
			Date DOB, String Address, String Phone_number, String Email) throws PersonException
	{
		...
		Date now = new Date();
		if(this.DOB.getYear() - now.getYear() > 1918) {
			throw new PersonException("Date entered: " + DOB + " is not valid");
		...
		if(matcher.matches() == true) {
			...
			throw new PersonException("Number entered: " + this.phone_number + " is not in a propper format.");
		}
		this.FirstName = FirstName;
		this.MiddleName = MiddleName;
		this.LastName = LastName;
